...
pub fn encode(src: &Trits) -> Vec<u8> {
    assert!(src.len() % TRITS_PER_BYTE == 0);
    src.iter_trytes()
        .zip(src[TRITS_PER_TRYTE..].iter_trytes())
        .map(|(a, b)| encode_group(a as i8, b as i8).unwrap() as u8)
        .collect()
}

fn encode_group(t1: i8, t2: i8) -> Option<i8> {
    i8::try_from(t1 as isize + t2 as isize * 27).ok()
}

/// Decode a series of bytes into trits.
pub fn decode<T: RawEncodingBuf>(bytes: &[u8]) -> TritBuf<T>
where
