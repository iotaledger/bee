...
use crate::{VoteObject, statement::OpinionStatement};

#[derive(Debug, thiserror::Error)]
pub enum Error {
    /// `OpinionStatement` already exists in `OpinionStatements` collection.
    #[error("Cannot add duplicate OpinionStatement {0:?}")]
    DuplicateOpinionStatement(OpinionStatement),
