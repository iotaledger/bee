...
use crate::{context::{VoteContext, VoteObject}, error::Error, events::{Event, OpinionEvent, RoundStats}, opinion::{Opinion, OpinionGiver, Opinions, QueriedOpinions, QueryObjects}};
    ...
    F: Fn() -> Vec<Box<dyn OpinionGiver>>,
{
    ...
    async fn query_opinions(&self) -> Result<Vec<QueriedOpinions>, Error> {
        ...
        let (vote_map, all_queried_opinions) = self.run_all_queries(
            &mut opinion_givers,
            query_objects,
            &selected_opinions,
        ).await;
    ...
    fn select_opinions(&self, opinion_givers: &Vec<Box<dyn OpinionGiver>>) -> Vec<u32> {
    ...
    async fn run_all_queries(
        ...
        selected_opinions: &Vec<u32>,
    ) -> (
        HashMap<VoteObject, Opinions>, 
        Vec<QueriedOpinions>
    ) {
        ...
        for (opinion_giver, opinion_select_count) in opinion_givers
            .iter_mut()
            .zip(selected_opinions.iter())
        {
    ...
    async fn do_query(
        ...
        let mut query_voting_objects = |vote_objects: &Vec<VoteObject>| {
            for (vote_object, opinion) in vote_objects
                .iter()
                .zip(opinions.iter()) 
            {
