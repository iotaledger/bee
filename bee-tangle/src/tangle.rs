...
use crate::{metadata::MessageMetadata, vertex::Vertex, MessageRef};
...
pub trait Hooks {
    ...
    fn get(&self, message_id: &MessageId) -> Result<Option<(Message, MessageMetadata)>, Self::Error>;
    /// Insert a message into some external storage medium.
    fn insert(&self, message_id: MessageId, msg: Message, metadata: MessageMetadata) -> Result<(), Self::Error>;
...
impl Hooks for NullHooks {
    ...
    fn get(&self, _message_id: &MessageId) -> Result<Option<(Message, MessageMetadata)>, Self::Error> {
        Ok(None)
    }

    fn insert(&self, _message_id: MessageId, _message: Message, _metadata: MessageMetadata) -> Result<(), Self::Error> {
        Ok(())
    }
...
impl<H: Hooks> Default for Tangle<H>
where
    H: Default,
{
    fn default() -> Self {
