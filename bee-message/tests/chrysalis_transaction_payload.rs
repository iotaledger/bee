...
mod cpt2 {
    use bee_message::{
        ...
        output::{unlock_condition::AddressUnlockCondition, BasicOutput, Output, SignatureLockedSingleOutput},
        payload::{
            cpt2::transaction::{
                ChrysalisRegularTransactionEssence, ChrysalisRegularTransactionEssenceBuilder,
                ChrysalisTransactionEssence, ChrysalisTransactionPayload,
            },
    ...
    #[test]
    fn kind() {
        assert_eq!(ChrysalisTransactionPayload::KIND, 0);
    }
    ...
    fn builder_no_essence_too_many_unlock_blocks() {
        ...
        let essence = ChrysalisTransactionEssence::Regular(
            ChrysalisRegularTransactionEssence::builder()
        ...
        let signature = Signature::Ed25519(Ed25519Signature::new(pub_key_bytes, sig_bytes));
        let sig_unlock_block = UnlockBlock::Signature(SignatureUnlockBlock::new(signature));
        ...
        let tx_payload = ChrysalisTransactionPayload::new(essence, unlock_blocks);
    ...
    fn pack_unpack_valid() {
        ...
        let tx_payload = ChrysalisTransactionPayload::new(essence, unlock_blocks).unwrap();
    ...
    fn getters() {
        ...
        let output = Output::SignatureLockedSingle(SignatureLockedSingleOutput::new(address, amount).unwrap());
        let essence = ChrysalisTransactionEssence::Regular(
            ChrysalisRegularTransactionEssenceBuilder::new()
                .with_inputs(vec![input1, input2])
                .with_outputs(vec![output])
                .finish()
                .unwrap(),
        ...
        let tx_payload = ChrysalisTransactionPayload::new(essence.clone(), unlock_blocks.clone()).unwrap();
