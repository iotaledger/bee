...
fn new_invalid_index_length_less_than_min() {
    assert!(matches!(
        ...
        Err(Error::InvalidIndexationIndexLength(TryIntoPrefixError::Invalid(
            InvalidBoundedU16(0)
        )))
    ));
}
...
fn new_invalid_data_length_more_than_max() {
    assert!(matches!(
        ...
        Err(Error::InvalidIndexationDataLength(TryIntoPrefixError::Invalid(InvalidBoundedU32(l)))) if l == u32::try_from(MESSAGE_LENGTH_MAX + 42).unwrap()
    ));
}
...
fn unpack_invalid_index_length_less_than_min() {
    assert!(matches!(
        ...
        Err(UnpackError::Packable(Error::InvalidIndexationIndexLength(
            TryIntoPrefixError::Invalid(InvalidBoundedU16(0))
        )))
    ));
}
...
fn unpack_invalid_index_length_more_than_max() {
    assert!(matches!(
        ...
        Err(UnpackError::Packable(Error::InvalidIndexationIndexLength(
            TryIntoPrefixError::Invalid(InvalidBoundedU16(65))
        )))
    ));
}
...
fn unpack_invalid_data_length_more_than_max() {
    assert!(matches!(
        ...
        Err(UnpackError::Packable(Error::InvalidIndexationDataLength(
            TryIntoPrefixError::Invalid(InvalidBoundedU32(33333))
        )))
    ));
}
