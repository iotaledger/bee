// Copyright 2020-2021 IOTA Stiftung
// SPDX-License-Identifier: Apache-2.0

use bee_message::{
    address::{Address, Ed25519Address},
    constant::IOTA_SUPPLY,
    output::SimpleOutput,
    Error,
};
use bee_packable::{bounded::InvalidBoundedU64, error::UnpackError, PackableExt};

use core::str::FromStr;

const ED25519_ADDRESS: &str = "52fdfc072182654f163f5f0f9a621d729566c74d10037c4d7bbb0407d1e2c649";

#[test]
fn kind() {
    assert_eq!(SimpleOutput::KIND, 0);
}

#[test]
fn new_valid_min_amount() {
    let address = Address::from(Ed25519Address::from_str(ED25519_ADDRESS).unwrap());
    let output = SimpleOutput::new(address, 1).unwrap();

    assert_eq!(*output.address(), address);
    assert_eq!(output.amount(), 1);
}

#[test]
fn new_valid_max_amount() {
    let address = Address::from(Ed25519Address::from_str(ED25519_ADDRESS).unwrap());
    let output = SimpleOutput::new(address, IOTA_SUPPLY).unwrap();

    assert_eq!(*output.address(), address);
    assert_eq!(output.amount(), IOTA_SUPPLY);
}

#[test]
fn new_invalid_less_than_min_amount() {
    assert!(matches!(
        SimpleOutput::new(Address::from(Ed25519Address::from_str(ED25519_ADDRESS).unwrap()), 0),
        Err(Error::InvalidAmount(InvalidBoundedU64(0)))
    ));
}

#[test]
fn new_invalid_more_than_max_amount() {
    assert!(matches!(
        SimpleOutput::new(
            Address::from(Ed25519Address::from_str(ED25519_ADDRESS).unwrap()),
            3_333_333_333_333_333
        ),
        Err(Error::InvalidAmount(InvalidBoundedU64(3_333_333_333_333_333)))
    ));
}

#[test]
fn packed_len() {
    let output = SimpleOutput::new(Address::from(Ed25519Address::from_str(ED25519_ADDRESS).unwrap()), 1).unwrap();

    assert_eq!(output.packed_len(), 1 + 32 + 8);
    assert_eq!(output.pack_to_vec().len(), 1 + 32 + 8);
}

#[test]
fn pack_unpack_valid() {
    let output_1 = SimpleOutput::new(Address::from(Ed25519Address::from_str(ED25519_ADDRESS).unwrap()), 1_000).unwrap();
    let output_2 = SimpleOutput::unpack_verified(&mut output_1.pack_to_vec().as_slice()).unwrap();

    assert_eq!(output_1, output_2);
}

#[test]
fn pack_unpack_invalid() {
    assert!(matches!(
        SimpleOutput::unpack_verified(
            &mut vec![
                0, 82, 253, 252, 7, 33, 130, 101, 79, 22, 63, 95, 15, 154, 98, 29, 114, 149, 102, 199, 77, 16, 3, 124,
                77, 123, 187, 4, 7, 209, 226, 198, 73, 0, 0, 0, 0, 0, 0, 0, 0,
            ]
            .as_slice()
        ),
        Err(UnpackError::Packable(Error::InvalidAmount(InvalidBoundedU64(0))))
    ));
}
