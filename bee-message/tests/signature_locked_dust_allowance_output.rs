...
fn new_invalid_less_than_min_amount() {
    assert!(matches!(
        ...
        Err(Error::InvalidDustAllowanceAmount(InvalidBoundedU64(999_999)))
    ));
}
...
fn pack_unpack_valid() {
    ...
    .unwrap();
    let output_2 = SignatureLockedDustAllowanceOutput::unpack_verified(&mut output_1.pack_to_vec().as_slice()).unwrap();
...
fn pack_unpack_invalid_amount() {
    assert!(matches!(
        SignatureLockedDustAllowanceOutput::unpack_verified(
            &mut vec![
