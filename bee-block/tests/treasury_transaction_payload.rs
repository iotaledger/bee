...
fn pack_unpack_valid() {
    ...
    .unwrap();
    let transaction_2 =
        TreasuryTransactionPayload::unpack_verified(&mut transaction_1.pack_to_vec().as_slice(), &mut ()).unwrap();
...
fn pack_unpack_invalid() {
    ...
    assert!(TreasuryTransactionPayload::unpack_verified(&mut bytes.as_slice(), &mut ()).is_err());
}
