...
use bee_block::{payload::milestone::option::TailTransactionHash, Error};
...
fn pack_unpack_valid() {
    ...
    let tth_2 = TailTransactionHash::unpack_verified(&mut tth_1_bytes.as_slice(), &mut ()).unwrap();
...
fn pack_unpack_invalid() {
    ...
    assert!(matches!(
        TailTransactionHash::unpack_verified(&mut bytes.as_slice(), &mut ()),
