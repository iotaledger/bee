...
impl PeerList {
    ...
    pub fn insert(&self, id: PeerId, info: PeerInfo, state: PeerState) -> Result<(), (PeerId, PeerInfo, Error)> {
        // TODO: unwrap
        if self.0.read().unwrap().contains_key(&id) {
            let short = id.short();
        ...
        // TODO: unwrap
        let _ = self.0.write().unwrap().insert(id, (info, state));
    ...
    pub fn send_message(&self, message: Vec<u8>, to: &PeerId) -> Result<(), Error> {
        // TODO: unwrap
        let this = self.0.read().unwrap();
        let (_, state) = this.get(to).ok_or(Error::UnlistedPeer(to.short()))?;
    ...
    pub fn count_if(&self, predicate: impl Fn(&PeerInfo, &PeerState) -> bool) -> usize {
        self.0.read().unwrap().iter().fold(0, |count, kv| {
    ...
    pub fn remove_if(&self, id: &PeerId, predicate: impl Fn(&PeerInfo, &PeerState) -> bool) {
        let remove = if let Some((info, state)) = self.0.read().unwrap().get(id) {
        ...
        if remove {
            let _ = self.0.write().unwrap().remove(id);
