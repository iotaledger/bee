...
mod peer;
mod peering;
// Code generated by `prost-build` derives `PartialEq` but not `Eq`.
// It is not possible to fix this upstream as generated code also deals with f64 that can't derive `Eq`.
// TODO: remove rustversion when derive_partial_eq_without_eq is stable.
#[rustversion::attr(not(stable), allow(clippy::derive_partial_eq_without_eq))]
mod proto {
